...
nb_matrix_fit <- function(
  ...
  if (impute_contact & contact_var %notin% c("c_ethnicity", "c_sec_input")) {
...
normalise_matrix <- function(matrix,
  ...
  scaled_matrix_1 <- matrix %>%
    arrange(bs_index, p_var) %>%
    left_join(population_dt, by = "p_var") %>%
    mutate(scaled_mu = mu * proportion) %>%
  ...
  scaled_matrix_2 <- scaled_matrix_1 %>%
    left_join(transposed_scaled_matrix_1, by = c("c_location", "bs_index", "c_var", "p_var")) %>%
    mutate(mean_scaled_mu = (scaled_mu + scaled_mu_t) / 2) %>%
    mutate(unscaled_mean_scaled_mu = mean_scaled_mu / proportion)

  matrix_out <- scaled_matrix_2 %>%
    select(c_location, bs_index, p_var, c_var, unscaled_mean_scaled_mu) %>%
    setnames("unscaled_mean_scaled_mu", "mu")

  if (per_capita == T) {
    matrix_out %>%
      arrange(bs_index, p_var) %>%
      left_join(population_dt %>% rename("c_var" = "p_var"), by = "c_var") %>%
      mutate(scaled_mu = mu / (proportion * sum(age_sex_strata$n))) %>%
      select(c_location, bs_index, p_var, c_var, scaled_mu) %>%
      rename("mu" = "scaled_mu")
  } else {
    matrix_out
  }
...
fcn_assortativity <- function(
  ...
  beta_hat <- beta_thin %>%
    left_join(n, by = "p_var") %>%
    mutate(beta_hat_var = mu * proportion)

  beta_hat$p_var <- factor(beta_hat$p_var, levels = order)
  beta_hat$c_var <- factor(beta_hat$c_var, levels = order)

  B <- beta_hat %>%
    left_join(beta_hat %>% group_by(!!!syms(bs_vars), c_var) %>% summarise(sum_beta_hat_var = sum(beta_hat_var)),
      by = c(bs_vars, "c_var")
    ) %>%
  ...
  Q <- B %>%
    filter(p_var == c_var) %>%
    group_by(!!!syms(bs_vars)) %>%
    summarise(sum_Bii = sum(B_var)) %>%
    mutate(Q_var = (sum_Bii - 1) / (n_distinct(beta_thin$p_var) - 1)) %>%
    select(!!!syms(bs_vars), Q_var) %>%
    filter(!is.na(Q_var))

  if (!is.null(bs_vars)) {
    q <- data.table(
      bs_index = unique(beta$bs_index),
      q_var = 0
    )
    ...
    for (bs in q$bs_index) {
      matrix <- beta_hat %>%
        filter(bs_index == bs) %>%
        arrange(p_var, c_var) %>%
        select(p_var, c_var, mu) %>%
        pivot_wider(id_cols = p_var, names_from = c_var, values_from = mu) %>%
        select(!p_var)
...
plot_mu_matrix <- function(matrix_data,
  ...
  if (p_var_name == "p_sec_input") {
    ...
    matrix_data$p_var <- fct_recode(matrix_data$p_var,
  ...
  if (c_var_name == "c_sec_input") {
    # remove 'Unknown' (first check all 0)
    if (sum(matrix_data[c_var == "Unknown"]$mu) != 0) {
      warning("sum(data[c_sec_input=='Unknown']$mu) != 0")
    }
    ...
    matrix_data$c_var <- fct_recode(matrix_data$c_var,
      "1. Higher managerial, administrative\nand professional occupations" = "1",
      "2. Lower managerial, administrative\nand professional occupations" = "2",
      "3. Intermediate occupations" = "3",
      "4. Small employers and\nown account workers" = "4",
  ...
  if (uncertainty == F) {
    matrix_data %>%
      ...
      labs(
        title = firstup(matrix_data$c_location[1]),
...
plot_and_save_mu_matrix <- function(matrix_data,
  ...
  plot_nb <- patchwork::wrap_plots(matrix_plots,
    design = layout,
